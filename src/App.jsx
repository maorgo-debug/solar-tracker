    ...
    <div style={{ background: "#161616", border: "1px solid #2a2a2a", borderRadius: 10, padding: "10px 14px", fontSize: 12 }}>
      ...
      {payload.map((p, i) => p.value != null && (
        <div key={i} style={{ color: p.color || "#fff", fontWeight: 600 }}>
          {p.name}: {fmt(p.value, 1)} {p.unit || "kWh"}
        </div>
      ))}
    </div>
  );
        ...
        const resp = await fetch("https://api.anthropic.com/v1/messages", {
          ...
          body: JSON.stringify({
            ...
            messages: [{ role: "user", content: [
              ...
              { type: "text", text: `Extract from SolarEdge screenshot. Return ONLY JSON:
{"kwh":<today kWh or null>,"current_kw":<kW now or null>,"time_hour":<decimal hour e.g 10.07 for 10:04 or null>,"date":"<YYYY-MM-DD or null>"}` }
            ]}]
          })
        });
  ...
  const totalValue = totalProduced * TARIFF;
  const loanRepaidPct = Math.min(100, (totalValue / LOAN_AMOUNT) * 100);
  const monthlyIncome = avgDaily * 30 * TARIFF;
  const annualIncome = avgDaily * 365 * TARIFF;
  ...
  const weekValue = weekTotal * TARIFF;
  const dailyChart = last7.map(e => ({ label: dateLabel(e.date), kwh: e.kwh, value: Math.round(e.kwh * TARIFF) }));
      ...
      {view === "week" && dailyChart.length > 0 && (
        <div style={{ background: "#0f0f0f", borderRadius: 14, padding: "18px 12px 12px", border: "1px solid #181818", marginBottom: 16 }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 14, paddingRight: 4 }}>
            ...
            <span style={{ color: "#2ecc71", fontSize: 11, fontWeight: 600 }}>{fmt(weekTotal, 0)} kWh · {fmt(weekValue, 0)} ₪</span>
          </div>
          <ResponsiveContainer width="100%" height={160}>
            <BarChart data={dailyChart} barSize={28}>
              ...
              <Bar dataKey="value" name="₪" radius={[5,5,0,0]} fill="transparent" />
            </BarChart>
          </ResponsiveContainer>
        </div>
      )}
